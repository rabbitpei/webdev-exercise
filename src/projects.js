...
$(function(){
	...
	e.preventDefault();
		...
		for(x in CURRENT_PROJECTS)  {
			if(CURRENT_PROJECTS[x].name==pj.name)  {
				resetForm($form);
				alert("This project is already in the projects list");
				return;
			}
		}
			
        $.ajax("https://api.github.com/repos/quixey/webdev-exercise/branches/"+pj.name)
          .done(function() {
	      	MAX_ID = pj.id;
